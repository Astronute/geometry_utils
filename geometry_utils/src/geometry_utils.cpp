...
namespace GU {
    ...
    // 将多边形顶点由数组表示转为单向链表表示
    Node* GeometryUtils::upgradePolygon(const std::vector<Point>& p) {
        Node* root = nullptr;
    ...
    Intersection calc_linesIntersect(const Line& lineA, const Line& lineB) {
        ...
        inc.alongA = C(0);
        inc.alongB = C(1);
        inc.p = Point(lineA.startX + inc.alongA * dx1, lineA.startY + inc.alongA * dy1);
    ...
    double pointInPolygon(const Point& p, Node* root) {
        ...
        do {
            ...
            Point v0_v1 = Point(Pnext->p.x - Pcur->p.x, Pnext->p.y - Pcur->p.y);
            Point v0_p = Point(p.x - Pcur->p.x, p.y - Pcur->p.y);
    ...
    std::vector<Point> simplifyCurve(const std::vector<Point>& curve, double epsilon) {
        ...
        if (get_index) {
            std::vector<Point> left_segment(curve.begin(), curve.begin() + index_target + 1);
            std::vector<Point> right_segment(curve.begin() + index_target, curve.end());
            std::vector<Point> dp_path_l = simplifyCurve(left_segment, epsilon);
            std::vector<Point> dp_path_r = simplifyCurve(right_segment, epsilon);
    ...
    bool calc_line_cross_polygon(const Line& line, const std::vector<Point>& polygon) {
        ...
        if (s_in * end_in == -1 || (s_in == 0 && end_in == 0)) {
            return true;
        }
    ...
    std::vector<std::vector<Point>> calc_AnotB(const std::vector<Point>& region_0, const std::vector<Point>& region_1) {
        ...
        std::vector<std::deque<Point>> polygons = selctor.segmentChain(sel_segments);
        for (auto polygon : polygons) {
            if (polygon.size() > 2) {
                std::vector<Point> polygon_vector(polygon.begin(), polygon.end());
